/**
 * Brute-force search for the triple crown: Said and Done!, Thailand Number One!
...
for (let r = 0; r < RESTARTS; r++) {
  ...
  let curR = robustScore(coalition, cur); evals += ENSEMBLE.length;
  for (let step = 0; step < STEPS; step++) {
    const cand = mutate(cur, rnd);
    const r2 = robustScore(coalition, cand); evals += ENSEMBLE.length;
