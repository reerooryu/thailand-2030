...
import {
  availableCards, applyFlags, lapsedProposals, initialFlags, attempt, effectiveEffects, isUnlocked,
...
export class Game {
  ...
  openTurn() {
    ...
    for (const c of lapsedProposals(this.cat, this.quarter, this.flags, this.ps.coalition)) {
  ...
  endTurn(exog: Exog) {
    ...
    const params: Params = { ...this.params,
      ...
      // Institutional support is not decoration. 'Others' is the establishment —
      // the bureaucracy, the provincial governors, the permanent secretaries who
      // decide whether a budget line becomes a contract this year or next. A
      // government they are cold toward disburses less of what it votes; one they
      // are warm toward disburses more. This is the channel through which every
      // institutionalSupport effect in the game finally reaches the economy.
      executionCapital: Math.min(1.0,
        (this.params.executionCapital + this.stance.executionBonus) * this.cooperation()) };
